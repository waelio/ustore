...
const isProcess = () => {
    try {
        return (typeof process !== undefined && process['browser']) ? process["browser"] : false;
    }
    catch (error) {
        return false;
    }
...
class Config {
    ...
    getUrgentOverrides() {
        let overrides;
        const filename = (0, exports.isProcess)() && ["production", 'prod'].includes(process.env.NODE_ENV) ? "prod" : "dev";
        try {
